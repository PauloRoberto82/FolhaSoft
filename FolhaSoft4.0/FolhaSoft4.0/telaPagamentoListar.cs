    ...
    public partial class telaPagamentoListar : Form
    {
        ...
        private void Buscar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //teto gerente
                string query = "SELECT fun.id_funcionario as [Código], fun.nome as [Nome], car.descricao_cargo as [Cargo], 'Ativo' as [Situação], CAST(pag.[valor_pagamento] AS DECIMAL(10, 2)) as [Salário], CAST(pag.[valor_pagamento] * CASE WHEN pag.[valor_pagamento] >= 0.01 AND pag.[valor_pagamento] < 1320 THEN 0.075 WHEN pag.[valor_pagamento] >= 1320 AND pag.[valor_pagamento] < 2571.29 THEN 0.09 WHEN pag.[valor_pagamento] >= 2571.29 AND pag.[valor_pagamento] < 3856.94 THEN 0.12 WHEN pag.[valor_pagamento] >= 3856.94 THEN 0.14 END AS DECIMAL(10, 2)) as [Valor Inss], CAST(pag.[valor_pagamento] * CASE WHEN pag.[valor_pagamento] >= 0.01 AND pag.[valor_pagamento] < 1903.99 THEN 0 WHEN pag.[valor_pagamento] >= 1903.99 AND pag.[valor_pagamento] < 2826.66 THEN 0.075 WHEN pag.[valor_pagamento] >= 2826.66 AND pag.[valor_pagamento] < 3751.05 THEN 0.15 WHEN pag.[valor_pagamento] >= 3751.05 AND pag.[valor_pagamento] < 4664.68 THEN 0.225 WHEN pag.[valor_pagamento] >= 4664.68 THEN 0.275 END AS DECIMAL(10, 2)) as [Valor Irrf], CAST( (pag.[valor_pagamento] - (pag.[valor_pagamento] * CASE WHEN pag.[valor_pagamento] >= 0.01 AND pag.[valor_pagamento] < 1320 THEN 0.075 WHEN pag.[valor_pagamento] >= 1320 AND pag.[valor_pagamento] < 2571.29 THEN 0.09 WHEN pag.[valor_pagamento] >= 2571.29 AND pag.[valor_pagamento] < 3856.94 THEN 0.12 WHEN pag.[valor_pagamento] >= 3856.94 THEN 0.14 END + CASE WHEN pag.[valor_pagamento] >= 0.01 AND pag.[valor_pagamento] < 1903.99 THEN 0 WHEN pag.[valor_pagamento] >= 1903.99 AND pag.[valor_pagamento] < 2826.66 THEN 0.075 WHEN pag.[valor_pagamento] >= 2826.66 AND pag.[valor_pagamento] < 3751.05 THEN 0.15 WHEN pag.[valor_pagamento] >= 3751.05 AND pag.[valor_pagamento] < 4664.68 THEN 0.225 WHEN pag.[valor_pagamento] >= 4664.68 THEN 0.275 END )) AS DECIMAL(10, 2)) as [Salário Líquido], pag.data_pagamento as [Data pagamento] FROM [projetoPIM].[dbo].[folha_pagamento] pag LEFT JOIN [dbo].[descontos] des ON (des.id_descontos = pag.id_descontos) LEFT JOIN [dbo].[funcionario] fun ON (fun.id_funcionario = pag.id_funcionario) LEFT JOIN [dbo].[cargos] car ON (fun.id_cargos = car.id_cargos) "+where+"";
                SqlCommand cmd = new SqlCommand(query, sqlConn);
                SqlDataAdapter dataAdapter = new SqlDataAdapter();
                dataAdapter.SelectCommand = cmd;
                DataTable dt = new DataTable();
